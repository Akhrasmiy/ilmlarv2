...
const {
  uploadTrailerToVimeo,
} = require("../../shared/uploads/uploadTrailerService.js");
const { default: axios } = require("axios");
const { imguploads } = require("../../shared/uploads/imgupload.js");
const { BadRequestError } = require("../../shared/errors/index.js");
async function generateRandomString(name) {
  name = name.toLowerCase();
  const slugify = (str) => {
    return str
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, "") // faqat harflar, raqamlar va bo‘shliqni qoldiradi
      .trim()
...
exports.createCourseService = async (data, file, trailerFile) => {
  ...
  console.log(data);
  const teacher = await db("teacher_more_date")
  ...
  const courseData = {
    ...
    status: 1,
    created_at: new Date(),
    idx: await generateRandomString(data.name),
  };
